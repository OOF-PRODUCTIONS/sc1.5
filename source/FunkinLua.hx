...
class FunkinLua {
	...
	public function new(script:String) {
		...
		Lua_helper.add_callback(lua, "resizeWindow", function(value1:Int, value2:Int) {
			Lib.application.window.resize(value1,value2);
		});
		...
		Lua_helper.add_callback(lua, "noteTweenScaleX", function(tag:String, note:Int, value:Dynamic, duration:Float, ease:String) {
			...
			if(testicle != null) {
				lePlayState.notes.forEach(function(spr:Note){
							if(note >= 4 && spr.mustPress && spr.noteData == (note - 4))
								{
							...
							else if (note <= 3 && !spr.mustPress && spr.noteData == note)
								{
									lePlayState.modchartTweens.set(tag, FlxTween.tween(spr, {"scale.x": value}, duration, {ease: getFlxEaseByString(ease),
										onComplete: function(twn:FlxTween) {
											lePlayState.callOnLuas('onTweenCompleted', [tag]);
		...
		Lua_helper.add_callback(lua, "warpMouse", function(valueX:Int, valueY:Int) {
			Lib.application.window.warpMouse(valueX, valueY);
		});
