...
class PreferencesSubstate extends MusicBeatSubstate
{
		...
		if(usesCheckbox) {
			if(controls.ACCEPT && nextAccept <= 0) {
				switch(options[curSelected]) {
					...
					case 'Random Botplay Text':
						ClientPrefs.randomBotplayText = !ClientPrefs.randomBotplayText;
					...
					case 'More Score Info':
						ClientPrefs.moreScoreInfo = !ClientPrefs.moreScoreInfo;
			...
			if(controls.UI_LEFT || controls.UI_RIGHT) {
				...
				switch(options[curSelected]) {
					...
					case 'Note Underlay Alpha':
						ClientPrefs.scrollUnderlayAlpha += addFloat;
						if(ClientPrefs.scrollUnderlayAlpha < 0) ClientPrefs.scrollUnderlayAlpha = 0;
						else if(ClientPrefs.scrollUnderlayAlpha >= 1.1) ClientPrefs.scrollUnderlayAlpha = 1;
				}
				reloadValues();
	...
	function changeSelection(change:Int = 0)
	{
		...
		switch(options[curSelected]) {
			...
			case 'Note Underlay Alpha':
				daText = "How transparent the underlay is.";
			case 'Note Splashes':
				daText = "If unchecked, hitting \"Sick!\" notes won't show particles.";
				case 'Eyesores':
					daText = "Uncheck this if you're sensitive to flashing colors!";
				case 'Anti-Spam':
					daText = "If unchecked, that stupid anti-spam won't bother you\nwhen beating Bambi's songs.";
				case 'Miss Sounds':
					daText = "If unchecked, missing notes won't make a miss sound.";
				case 'More Score Info':
					daText = "If checked, you will see your \"Sicks!\", \"Goods\",\n\"Bads\", \"Shits\", combo, and total notes hit.";
			case 'Flashing Lights':
				daText = "Uncheck this if you're sensitive to flashing lights!";
			case 'Window Moving':
				daText = "If checked, the window will move in a few songs.\nDisable if you get motion sickness.";
			case 'Hit Sounds':
				daText = "If checked, hit sounds will play when you hit a note.";
			case 'Camera Zooms':
			...
			case 'Random Botplay Text':
				daText = "Disable if you don't want to be insulted\neverytime you use botplay.";
		}
		...
		for (i in 0...checkboxArray.length) {
			...
			if(checkbox != null) {
				...
				switch(options[checkboxNumber[i]]) {
					...
					case 'Random Botplay Text':
						daValue = ClientPrefs.randomBotplayText;
					case 'Anti-Aliasing':
					...
					case 'Hide Song Length':
						...
						case 'More Score Info':
							daValue = ClientPrefs.moreScoreInfo;
				}
		...
		for (i in 0...grpTexts.members.length) {
			...
			if(text != null) {
				...
				switch(options[textNumber[i]]) {
					...
					case 'Note Underlay Alpha':
						daText = '' + ClientPrefs.scrollUnderlayAlpha;
				}
