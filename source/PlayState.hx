...
class PlayState extends MusicBeatState
{
	...
	var hadephobiaColor:FlxColor = 0xFF474747;
	...
	var windowMovingSongs:Array<String> = [];


	var halloweenBG:BGSprite;
	var halloweenWhite:BGSprite;
	var tweenerBool:Bool = false;
	...
	var badCounter:Int;
	var shitCounter:Int;
	var glitchyEffect:FlxGlitchEffect;
	...
	public var darkLevels:Array<String> = ['farmNight'];

	var tnh:Int = 0;

	var movingWindowSongs:Array<String> = ['ornery', 'hellbreaker', 'opposition'];
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'yoyleworld': //Week 1
				var oppositonBG:BGSprite = new BGSprite('bambi/oppositon/OppositonLol', -600, -300, 0, 0);
				oppositonBG.setGraphicSize(Std.int(oppositonBG.width * 1.8));
				oppositonBG.setGraphicSize(Std.int(oppositonBG.height * 1.8));
				oppositonBG.shader = the3DWorldEffectFlag.shader;
				add(oppositonBG);

				var oppositonGround:BGSprite = new BGSprite('bambi/oppositon/ThanosGround', 790, 620, 1, 1);
			...
			case 'spammer': //Week 1
				var spamBG:BGSprite = new BGSprite('bambi/spam', -575, -500, 0, 0);
				spamBG.setGraphicSize(Std.int(spamBG.width * 1.8));
				spamBG.setGraphicSize(Std.int(spamBG.height * 1.8));
				spamBG.shader = the3DWorldEffectFlag.shader;
				add(spamBG);

			case 'theoreticaldimension': //Week 1
				var theorBG:BGSprite = new BGSprite('bambi/theorbg', -650, -320, 0, 0);
				theorBG.setGraphicSize(Std.int(theorBG.width * 1.8));
				theorBG.setGraphicSize(Std.int(theorBG.height * 1.8));
				theorBG.shader = the3DWorldEffectFlag.shader;
				add(theorBG);

			case 'haxxer': //Week 1
				var haxxerBG:BGSprite = new BGSprite('bambi/haxxer/ornerybg', -714, -354, 0, 0);
				haxxerBG.setGraphicSize(Std.int(haxxerBG.width * 1.8));
				haxxerBG.setGraphicSize(Std.int(haxxerBG.height * 1.8));
				haxxerBG.shader = the3DWorldEffectWavy.shader;
				add(haxxerBG);
				
				var cubes:BGSprite = new BGSprite('bambi/haxxer/Cubespin', -800, -284, 0, 0, ['Speen'], true);
				cubes.animation.addByPrefix('Speen', 'Speen', 30, true);
				cubes.setGraphicSize(Std.int(cubes.width * 1.8));
				cubes.setGraphicSize(Std.int(cubes.height * 1.8));
				cubes.shader = the3DWorldEffectWavy.shader;
				add(cubes);
				cubes.animation.play('Speen');

			case 'fatherlylove': //Week 1
				var fatherlyloveBG:BGSprite = new BGSprite('bambi/fatherlylove', -300, -300, 0, 0);
				fatherlyloveBG.setGraphicSize(Std.int(fatherlyloveBG.width * 1.8));
				fatherlyloveBG.setGraphicSize(Std.int(fatherlyloveBG.height * 1.8));
				fatherlyloveBG.shader = the3DWorldEffectFlag.shader;
				add(fatherlyloveBG);


			case 'scarybg': //Week 1
				var scaryBG:BGSprite = new BGSprite('bambi/scarybg/scarybg', -280, -130, 0, 0);
				scaryBG.setGraphicSize(Std.int(scaryBG.width * 1.8));
				scaryBG.setGraphicSize(Std.int(scaryBG.height * 1.8));
				scaryBG.shader = the3DWorldEffectFlag.shader;
				add(scaryBG);
			case 'cheater': //Week 1
				var cheaterBG:BGSprite = new BGSprite('bambi/cheater', -280, -100, 0, 0);
				cheaterBG.setGraphicSize(Std.int(cheaterBG.width * 1.8));
				cheaterBG.setGraphicSize(Std.int(cheaterBG.height * 1.8));
				cheaterBG.shader = the3DWorldEffectFlag.shader;
				add(cheaterBG);
			case 'golden': //Week 1
				var goldenBG:BGSprite = new BGSprite('bambi/bumbleybg', 370, -50, 0, 0);
				goldenBG.setGraphicSize(Std.int(goldenBG.width * 6));
				goldenBG.setGraphicSize(Std.int(goldenBG.height * 2.5));
				goldenBG.shader = the3DWorldEffectFlag.shader;
				add(goldenBG);
			case 'PTHbg': //Week 1
				var pthBG:BGSprite = new BGSprite('bambi/PTH_BG', -280, -50, 0, 0);
				pthBG.setGraphicSize(Std.int(pthBG.width * 2.5));
				pthBG.setGraphicSize(Std.int(pthBG.height * 2.5));
				pthBG.shader = the3DWorldEffectFlag.shader;
				add(pthBG);
			case 'unfairness':
				var greenSkyUnfairness:BGSprite = new BGSprite('bambi/unfairness', -400, -400, 0, 0);
				greenSkyUnfairness.setGraphicSize(Std.int(greenSkyUnfairness.width * 2));
				greenSkyUnfairness.setGraphicSize(Std.int(greenSkyUnfairness.height * 2));
				greenSkyUnfairness.updateHitbox();
				add(greenSkyUnfairness);
				greenSkyUnfairness.shader = the3DWorldEffectFlag.shader;
			case 'cone': //Week 1
				var coneBG:BGSprite = new BGSprite('bambi/cone', -280, -50, 0, 0);
				coneBG.setGraphicSize(Std.int(coneBG.width * 2.5));
				coneBG.setGraphicSize(Std.int(coneBG.height * 2.5));
				coneBG.shader = the3DWorldEffectFlag.shader;
				add(coneBG);
			case 'hadephobia': //Week 1
				var hadephobiaBG:BGSprite = new BGSprite('bambi/HADEPHOBIABG', -280, -50, 0, 0);
				hadephobiaBG.setGraphicSize(Std.int(hadephobiaBG.width * 2.5));
				hadephobiaBG.setGraphicSize(Std.int(hadephobiaBG.height * 2.5));
				hadephobiaBG.shader = the3DWorldEffectFlag.shader;
				add(hadephobiaBG);
			case 'obscure': //Week 1
				var amogusBG:BGSprite = new BGSprite('bambi/amogus', -280, -50, 0, 0);
				amogusBG.setGraphicSize(Std.int(amogusBG.width * 2.5));
				amogusBG.setGraphicSize(Std.int(amogusBG.height * 2.5));
				amogusBG.shader = the3DWorldEffectFlag.shader;
				add(amogusBG);
			case 'futile': //Week 1
				var futileBG:BGSprite = new BGSprite('bambi/futileBG', -280, -50, 0, 0);
				futileBG.setGraphicSize(Std.int(futileBG.width * 2.5));
				futileBG.setGraphicSize(Std.int(futileBG.height * 2.5));
				futileBG.shader = the3DWorldEffectFlag.shader;
				add(futileBG);
			case 'nightfarm': //Week 1
				var theBGNight:BGSprite = new BGSprite('sky_night', -680, -130, 0, 0);
				add(theBGNight);
				var hillsNight:BGSprite = new BGSprite('orangey hills', -380, 160, 0.3, 0.3);
				add(hillsNight);
				hillsNight.color = nightColor;
				var farmNight:BGSprite = new BGSprite('funfarmhouse', 80, 200, 0.6, 0.6);
				add(farmNight);
				farmNight.color = nightColor;
				var groundNight:BGSprite = new BGSprite('grass lands', -480, 600, 1, 1);
				add(groundNight);
				groundNight.color = nightColor;
				var corn1night:BGSprite = new BGSprite('Cornys', -360, 325, 1, 1);
				add(corn1night);
				corn1night.color = nightColor;
				var corn2night:BGSprite = new BGSprite('Cornys', 1060, 325, 1, 1);
				add(corn2night);
				corn2night.color = nightColor;
				var fenceNight:BGSprite = new BGSprite('crazy fences', -350, 450, 1, 1);
				add(fenceNight);
				fenceNight.color = nightColor;
				var signNight:BGSprite = new BGSprite('Sign', 10, 495, 1, 1);
				add(signNight);
				signNight.color = nightColor;
			case 'trippy farm': //Week 1
				var trippyBG:BGSprite = new BGSprite('W', -280, -130, 1, 1);
				trippyBG.setGraphicSize(Std.int(trippyBG.width * 2.5));
				trippyBG.setGraphicSize(Std.int(trippyBG.height * 2.5));
				_effectSprite = new FlxEffectSprite(trippyBG, [_glitch]);
				add(trippyBG);
			case 'space':
				var stars:BGSprite = new BGSprite('bamb/Bg_Stars', -330, -100, 0, 0);
				stars.setGraphicSize(Std.int(stars.width * 4));
				stars.setGraphicSize(Std.int(stars.height * 4));
				stars.shader = the3DWorldEffectFlag.shader;
				add(stars);
				var planet:BGSprite = new BGSprite('bamb/Bg_Planet', 2000, -80, 0.5, 0.5);
				planet.setGraphicSize(Std.int(planet.width * 2.5));
				planet.setGraphicSize(Std.int(planet.height * 2.5));
				add(planet);
				var cubes:BGSprite = new BGSprite('bamb/Bg_Cubes', stars.x - 150, stars.y, 0.85, 0.85);
				cubes.setGraphicSize(Std.int(cubes.width * 4));
				cubes.setGraphicSize(Std.int(cubes.height * 4));
				cubes.shader = the3DWorldEffectWavy.shader;
				add(cubes);
				FlxTween.tween(planet, {x:-6000}, 25, {type: FlxTweenType.LOOPING, ease: FlxEase.linear});
			case 'automate':
				var automateBG:BGSprite = new BGSprite('bumbo/automate', -280, -50, 0, 0);
				automateBG.setGraphicSize(Std.int(automateBG.width * 2.5));
				automateBG.setGraphicSize(Std.int(automateBG.height * 2.5));
				automateBG.shader = the3DWorldEffectFlag.shader;
				add(automateBG);
			case 'spambi':
				var spambigradient:BGSprite = new BGSprite('spambi/spambigradient', -280, -50, 0, 0);
				spambigradient.setGraphicSize(Std.int(spambigradient.width * 4));
				spambigradient.setGraphicSize(Std.int(spambigradient.height * 4));
				spambigradient.shader = the3DWorldEffectFlag.shader;
				spambigradient.screenCenter(XY);
				add(spambigradient);
				var spambihats:BGSprite = new BGSprite('spambi/spambihats', spambigradient.x - 150, spambigradient.y, 0.3, 0.3);
				spambihats.setGraphicSize(Std.int(spambihats.width * 4));
				spambihats.setGraphicSize(Std.int(spambihats.height * 4));
				spambihats.shader = the3DWorldEffectWavy.shader;
				spambihats.screenCenter(XY);
				add(spambihats);
			case 'diambi':
				var diambi:BGSprite = new BGSprite('diambi/New_Diambi_BG', -280, -50, 0, 0);
				diambi.setGraphicSize(Std.int(diambi.width * 2));
				diambi.setGraphicSize(Std.int(diambi.height * 2));
				diambi.shader = the3DWorldEffectFlag.shader;
				diambi.screenCenter(XY);
				add(diambi);
			case 'metaphysical':
				var diambi:BGSprite = new BGSprite('metabg', -280, -50, 0, 0);
				diambi.setGraphicSize(Std.int(diambi.width * 2));
				diambi.setGraphicSize(Std.int(diambi.height * 2));
				diambi.shader = the3DWorldEffectFlag.shader;
				diambi.screenCenter(XY);
				add(diambi);
			case 'idiot':
				var diambi:BGSprite = new BGSprite('idiot3dbackground', -280, -50, 0, 0);
				diambi.setGraphicSize(Std.int(diambi.width * 2));
				diambi.setGraphicSize(Std.int(diambi.height * 2));
				diambi.shader = the3DWorldEffectFlag.shader;
				diambi.screenCenter(XY);
		...
		Conductor.songPosition = -5000;
			...
			{
				var noteUnderlay:FlxSprite = new FlxSprite(395, 0).makeGraphic(480, FlxG.height, FlxColor.BLACK);
				noteUnderlay.alpha = ClientPrefs.scrollUnderlayAlpha;
				add(noteUnderlay);
				noteUnderlay.cameras = [camHUD];
			}
			else
			{
				var noteUnderlay:FlxSprite = new FlxSprite(714, 0).makeGraphic(480, FlxG.height, FlxColor.BLACK);
				noteUnderlay.alpha = ClientPrefs.scrollUnderlayAlpha;
				add(noteUnderlay);
				noteUnderlay.cameras = [camHUD];
			}
		...
		scoreTxt.setFormat(Paths.font("comicsanslol.ttf"), 20, FlxColor.LIME, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		if(SONG.credit != null) {
			...
			watermarkTxt.setFormat(Paths.font("comicsanslol.ttf"), 16, FlxColor.LIME, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		else if(songName == 'unfairness')
			{
				watermarkTxt.y = FlxG.height - 50;
				screwYouTxt = new FlxText(10, FlxG.height - 28, 0, "Ghost Tapping is forced off! Screw you!", 74);
				screwYouTxt.scrollFactor.set();
				screwYouTxt.setFormat(Paths.font("comicsanslol.ttf"), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				screwYouTxt.size = 18;
				screwYouTxt.updateHitbox();
				screwYouTxt.alpha = 0.6;
				add(screwYouTxt);
			}
		else if(songName == 'cheating')
			{
				watermarkTxt.y = FlxG.height - 50;
				screwYouTxt = new FlxText(10, FlxG.height - 28, 0, "Screw you!", 74);
				screwYouTxt.scrollFactor.set();
				screwYouTxt.setFormat(Paths.font("comicsanslol.ttf"), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				screwYouTxt.size = 18;
				screwYouTxt.updateHitbox();
		...
		else if(songName == 'obscure')
			{
				watermarkTxt.y = FlxG.height - 50;
				screwYouTxt = new FlxText(10, FlxG.height - 28, 0, "sussus amogus", 74);
				screwYouTxt.scrollFactor.set();
				screwYouTxt.setFormat(Paths.font("comicsanslol.ttf"), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				screwYouTxt.size = 18;
				screwYouTxt.updateHitbox();
		...
		else if(songName == 'opposition')
			{
				
				watermarkTxt.y = FlxG.height - 50;
				screwYouTxt = new FlxText(10, FlxG.height - 28, 0, "Screw you! You're fucked lol!", 74);
				screwYouTxt.scrollFactor.set();
				screwYouTxt.setFormat(Paths.font("comicsanslol.ttf"), 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				screwYouTxt.size = 18;
				screwYouTxt.updateHitbox();
		...
		if(SONG.screwYou != null)
			{
				...
				screwYouTxt.scrollFactor.set();
				screwYouTxt.setFormat(Paths.font("comicsanslol.ttf"), 16, FlxColor.LIME, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				screwYouTxt.size = 18;
				screwYouTxt.updateHitbox();
				screwYouTxt.alpha = 0.6;
				add(screwYouTxt);
		...
		sickCounterTxt.setFormat(Paths.font("comicsanslol.ttf"), 16, FlxColor.LIME, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		goodCounterTxt.setFormat(Paths.font("comicsanslol.ttf"), 16, FlxColor.LIME, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		goodCounterTxt.size = 18;
		add(goodCounterTxt);
		badCounterTxt = new FlxText(10, FlxG.height - 164, 0, "Bads: " + badCounter, 50);
		badCounterTxt.scrollFactor.set();
		badCounterTxt.setFormat(Paths.font("comicsanslol.ttf"), 16, FlxColor.LIME, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		badCounterTxt.size = 18;
		add(badCounterTxt);
		shitCounterTxt = new FlxText(10, FlxG.height - 144, 0, "Shits: " + shitCounter, 50);
		shitCounterTxt.scrollFactor.set();
		shitCounterTxt.setFormat(Paths.font("comicsanslol.ttf"), 16, FlxColor.LIME, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		shitCounterTxt.size = 18;
		add(shitCounterTxt);

		comboCounterTxt = new FlxText(10, FlxG.height - 244, 0, "Combo: " + combo, 70);
		comboCounterTxt.scrollFactor.set();
		comboCounterTxt.setFormat(Paths.font("comicsanslol.ttf"), 16, FlxColor.LIME, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		totalNotesTxt.setFormat(Paths.font("comicsanslol.ttf"), 16, FlxColor.LIME, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		if(!ClientPrefs.moreScoreInfo)
			{
				sickCounterTxt.alpha = 0;
				goodCounterTxt.alpha = 0;
				badCounterTxt.alpha = 0;
				shitCounterTxt.alpha = 0;
				comboCounterTxt.alpha = 0;
		...
		//PRECACHING MISS SOUNDS BECAUSE I THINK THEY CAN LAG PEOPLE AND FUCK THEM UP IDK HOW HAXE WORKS // (well fuck you)
	...
	override public function update(elapsed:Float)
	{
		...
		switch (curStage)
		{
			...
			case 'futile':
				the3DWorldEffectFlag.update(elapsed);
			case 'trippy farm':
				_effectSprite.update(elapsed);
				_glitch.update(elapsed);
			case 'space':
				if(!planetSpawned) {
				}
...
Lib.application.window.resize(width, height);*/
		...
		if(ratingString == '?') {
			scoreTxt.text = 'Score: ' + songScore + ' | Phone Breaks: ' + songMisses + ' | Rating: ' + ratingString;
		} else {
		...
		{
			if (Paths.formatToSongPath(SONG.song) == 'obscure')
				{
					if(leDate.getMonth() == 11 && leDate.getDate() >= 19 && leDate.getDate() <= 24 && curStep >= 1)
						{
						...
						if(storyWeek == 3)
							{
								if(storyDifficulty >= 1)
								{
									FlxG.save.data.orneryUnlocked1 = true;
									FlxG.save.data.spamophobiaUnlocked1 = true;
									trace("unlocked two songs");
								}
							}
					}
		...
		if (!note.wasGoodHit)
		{
			...
			if(note.hitCausesMiss) {
				...
				switch(note.noteType) {
					...
					case 'Static Note': //Hurt note
						...
						if(neverGotJumped)
						{
						var jumpscaredstatic:BGSprite = new BGSprite('staticjump', 0, 0, 0, 0, ['jumpscare'], true);
						jumpscaredstatic.animation.addByPrefix('jumpscare', 'jumpscare', 24, true);
						jumpscaredstatic.setGraphicSize(Std.int(jumpscaredstatic.width * 1.8));
						jumpscaredstatic.setGraphicSize(Std.int(jumpscaredstatic.height * 1.8));
						remove(jumpscaredstatic);
						add(jumpscaredstatic);
						jumpscaredstatic.alpha = 1;
						jumpscaredstatic.animation.play('jumpscare');
						jumpscaredstatic.alpha = 1;
						FlxTween.tween(jumpscaredstatic, {alpha:0}, 1, {ease: FlxEase.linear});
						jumpscaredstatic.cameras = [camHUD];
						}
			...
			if (!note.isSustainNote)
			{
				...
				if(ClientPrefs.hitSounds)
					{
					FlxG.sound.play(Paths.sound('note_click'), 1);
					}
			}
	...
	override function stepHit()
	{
		...
		if(textIsGoing)
			{
				screwYouTxt.text = textStuffLol[FlxG.random.int(0, textStuffLol.length - 1)];
			}
	...
	public function RecalculateRating() {
		...
		setOnLuas('phone breaks', songMisses);
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			ratingFC = "";
			if (sickCounter > 0) ratingFC = "SFC";
			if (goodCounter > 0) ratingFC = "GFC";
			if (badCounter > 0 || shitCounter > 0) ratingFC = "FC";
...
//guys be careful bambi is under here
